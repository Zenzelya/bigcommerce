...
const query = 'query ExtendedProductsById { site { products(first: 49) { edges { node { name sku images { edges { node { urlOriginal } } } prices { basePrice { value currencyCode } } description } } } } }';
export default class BulkOrder extends PageManager {
    ...
    getProductsSKu() {
        const customerId = `${this.context.shopperId}`;
        let productsSKU = [];
        const customersData = $('#page-content > *').text().split(';');
        ...
        customersData.forEach((item) => {
            if (item.split(',').includes(customerId)) {
                productsSKU = item.split(',');
                productsSKU.shift();
            }
        });

        return !productsSKU.length ?
            null :
            productsSKU;
    }
